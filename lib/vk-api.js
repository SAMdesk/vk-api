...
function VK(options, redirectUri, code, token) {
    ...
    self.userId = 0;
    self.timeout = options.timeout || 5000;
}
...
VK.prototype = {
    ...
    _apiCall: function (method, params, callback) {
        ...
        self._request(url, function(err, res, data) {
            if (err) return callback(err);
            var json = null;
            try {
                json = JSON.parse(data);
            } catch(e) {
                let err = {type: 'json_exception', message: 'could not parse JSON', data: data};
                self.emit('apiCall', err, null);
    ...
    init: function (callback) {
        ...
        self._request(url, function(err, res, data) {
            ...
            if (res.statusCode > 200) {
                self.emit('initError', {error_code: res.statusCode, error_msg: data});
                callback({error_code: res.statusCode, error_msg: data});
            } else {
    ...
    _request: function(url, callback) {
        ...
        var timeout = setTimeout(function() {
            req.abort();
            timed_out = true;
            callback({ type: 'timeout_error', message: 'The request to VK has timed out. Please try again in a few minutes.' });
        }, self.timeout);
        ...
        req.on('error', function() {
            if (!timed_out) {
                clearTimeout(timeout);
                callback({ type: 'connection_error', message: 'There was an error connecting to the VK servers. Please try again in a few minutes.' });
            }
        });
